...
interface DecodingDenPhonemeKeyboardProps {
...
export default function DecodingDenPhonemeKeyboard({ selectedLetters, setSelectedLetters, currentPosition, setCurrentPosition, draggedLetters, setDraggedLetters, nextZIndex, setNextZIndex, wordsOnPaper, dragOverLine, handleLineDrop, handleLineDragOver, handleLineDragLeave, clearWritingPaper }: DecodingDenPhonemeKeyboardProps) {
  ...
  useEffect(() => {
    ...
    console.log('Loaded mascot from localStorage:', savedMascot);
    if (savedMascot) {
      setSelectedMascot(savedMascot);
    }
  }, []);

  // Debug the current selectedMascot
  useEffect(() => {
    console.log('Current selectedMascot in Word List:', selectedMascot);
  }, [selectedMascot]);
  ...
  return (
    <div className="max-w-7xl mx-auto p-6 -ml-8 relative">
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        {/* Left Column - Interactive Keyboard (2/3 width) */}
        <div className="lg:col-span-2" style={{marginTop: "-20px"}}>

          {/* Alphabet Keyboard */}
          <div className="bg-white rounded-xl shadow-lg p-2 inline-block">
            <div className="flex flex-wrap gap-2 justify-center" style={{width: "680px"}}>
            {Object.values(phonemeData).map((data) => (
              ...
              >
                <div className="bg-gradient-to-br from-blue-500 to-green-500 text-white rounded-lg hover:from-blue-600 hover:to-green-600 transition-all duration-200 transform hover:scale-105 shadow-md hover:shadow-lg select-none flex items-center justify-center" 
                  style={{width: "68px", height: "68px", outline: "2px solid black"}}>
                  <div className={`text-5xl ${['a', 'e', 'i', 'o', 'u'].includes(data.letter) ? 'text-red-500 drop-shadow-lg' : 'text-white'}`} style={['a', 'e', 'i', 'o', 'u'].includes(data.letter) ? {WebkitTextStroke: '2px black'} : {WebkitTextStroke: '2px black'}}>{data.letter}</div>
            ...
            <button
              ...
              className="bg-gradient-to-b from-gray-400 to-gray-600 text-white rounded-lg p-4 hover:from-gray-500 hover:to-gray-700 transition-all duration-200 transform hover:scale-105 shadow-md hover:shadow-lg text-center flex flex-col items-center justify-center"
              style={{width: "68px", height: "68px", outline: "2px solid black"}}
            >
              <div className="text-2xl font-bold mb-1">
                {showPhoneme ? '👀' : (
                  <svg width="38" height="19" viewBox="0 0 32 16" fill="none" className="mx-auto">
                    {/* Left closed eye */}
        ...
        {/* Right Column - My Word List */}
        <div 
          className="bg-white rounded-xl shadow-2xl border-2 border-blue-500 pt-2 px-2 pb-2 relative overflow-visible" 
          style={{
            marginTop: "-200px",
            width: "280px",
            marginLeft: "100px"
        ...
        >
          <div className="flex justify-between items-start mb-0">
            <h2 className="text-2xl font-bold text-oceanBlue pt-4 -mb-2">
              Word List
            </h2>
            <button
              onClick={() => {
              ...
              className="hover:scale-110 transition-transform duration-200 cursor-pointer p-2 mt-0"
          ...
          {/* Lined Paper */}
          <div className="h-[320px] w-full bg-white rounded-lg relative">
            {/* Paper lines */}
                ...
                return (
                  <div key={index}>
                    ...
                    <div 
                      ...
                      style={{ top: `${index === 0 ? 25 : 30 + (index * 38)}px` }}
                    ></div>
                      ...
                      >
                        ...
                        {wordsOnPaper.find(w => w.lineIndex === index - 1) && (
                          <div 
                            ...
                            style={{ top: '2px' }}
                          >
                            <div className="bg-gradient-to-br from-orange-500 to-yellow-500 text-black rounded-lg shadow-xl flex items-center justify-center text-xl font-bold select-none px-2 py-1"
                              style={{outline: "2px solid black", minWidth: "65px", height: "32px"}}>
